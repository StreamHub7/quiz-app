    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (videoToken) {
          ...
          const streamDatas = await fs.readFile('url.json', 'utf8');
          const streamData = JSON.parse(streamDatas);
          const channelData = streamData.find(data => data.id === channel_ID);
          if (channelData) {
            channelData.videoUrl = videoToken;
            streamData[streamData.indexOf(channelData)] = channelData;
            await fs.writeFile('url.json', JSON.stringify(streamData));
          }
          else {
            streamData.push({ id: channel_ID, videoUrl: videoToken });
            await fs.writeFile('url.json', JSON.stringify(streamData));
          }
        }
      }
...
app.get('/index.php', async (req, res) => {
  try {
      ...
      const streamData = JSON.parse(streamDatas);
      const channelData = streamData.find(data => data["id"] === channel_ID);
      if (channelData && channelData.videoUrl) {
        res.redirect(channelData.videoUrl);
      } else {
